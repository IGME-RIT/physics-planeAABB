...
struct AABB
{
	...
	///
	//Generates an AABB of unit length
	AABB::AABB()
	{
		width = 2.0f;
		height = 2.0f;
	...
	///
	//Generates an AABB of a given width and height
...
///
//Tests for collisions between a plane and an axis aligned bounding box.
//
//Overview:
...
//	Because the box is axis aligned we do not need to worry about the orientation of the box, only the position and scale.
...
//	origin of the system to be on the plane by translating both the plane and the AABB by the negative of the plane's position.
//
//Parameters:
//	aabb: The axis aligned bounding box being tested for collision
//	aabbTrans: The translation transformation matrix of the axis aligned bounding box
//	aabbScale: The scale transformation matrix of the axis aligned bounding box
...
//	else false
bool TestCollision(
	const AABB &aabb, const glm::mat4 &aabbTrans, const glm::mat4 &aabbScale,
	const Plane &plane, const glm::mat4 &planeTrans, const glm::mat4 &planeRotation)
{
	//Step 1: We must construct a list of the corners of the AABB in model space
	std::vector<glm::vec4> corners;

	corners.push_back(glm::vec4(aabb.width  / 2.0f, aabb.height  / 2.0f, aabb.depth  / 2.0f, 1.0f));
	corners.push_back(glm::vec4(-aabb.width / 2.0f, aabb.height  / 2.0f, aabb.depth  / 2.0f, 1.0f));
	corners.push_back(glm::vec4(-aabb.width / 2.0f, aabb.height  / 2.0f, -aabb.depth / 2.0f, 1.0f));
	corners.push_back(glm::vec4(aabb.width  / 2.0f, aabb.height  / 2.0f, -aabb.depth / 2.0f, 1.0f));
	corners.push_back(glm::vec4(aabb.width  / 2.0f, -aabb.height / 2.0f, aabb.depth  / 2.0f, 1.0f));
	...
	glm::mat4 transform = glm::translate(aabbTrans, glm::vec3(-planeTrans[3][0], -planeTrans[3][1], -planeTrans[3][2])) * aabbScale;
...
void update()
{
	...
	if (TestCollision(*boxCollider, box->translation, box->scale, *planeCollider, plane->translation, plane->rotation))
	{
		//Turn red on
		hue[0][0] = 1.0f;
	}
...
void main()
{
	...
	// Creates a window
	window = glfwCreateWindow(800, 800, "Plane - AABB", nullptr, nullptr);
	...
	boxCollider = new struct AABB(boxVerts[3].x - boxVerts[2].x, boxVerts[9].y - boxVerts[8].y, boxVerts[1].z - boxVerts[0].z);
